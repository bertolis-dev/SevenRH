...
--
-- IMPORTANT — identifiants en `text`, pas `uuid` : l'application génère ses propres identifiants
-- via generateId(prefix) (ex. "emp_1785133863658_abc123"), qui ne sont PAS des UUID valides. Seule
-- `employees.auth_user_id` reste un vrai `uuid` (référence auth.users, propre à Supabase Auth).
--
...
create table employees (
  ...
  auth_user_id uuid references auth.users(id), -- rempli en Phase 3 (authentification réelle) — vrai uuid Supabase Auth
  email text not null,
...
create table favorites (
  ...
  favorite_employee_id text not null references employees(id) on delete cascade,
